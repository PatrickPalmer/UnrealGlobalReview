...
def log_entry(id:str, content:Any, *, region=None):

    log_group_name = os.environ.get("LOG_GROUP_NAME")

    if 'AWS_DEPLOYMENT_REGION' in os.environ:
        deployment_region = os.environ.get('AWS_DEPLOYMENT_REGION')
    else:
    ...
    log = {
        ...
        "Active": True,
        'CreateTime': isonow + "Z",
        'Region': region if region else os.environ.get('AWS_REGION'),
    }
    ...
    log_client = boto3.client('logs', region_name=deployment_region)
    ...
    try:
        ...
        # thrown exception if log stream already exists
        pass
...
def get_log_entries(id:str, *, region=None):
    log_group_name = os.environ.get("LOG_GROUP_NAME")

    if region:
        deployment_region = region
    elif 'AWS_DEPLOYMENT_REGION' in os.environ:
        deployment_region = os.environ.get('AWS_DEPLOYMENT_REGION')
    else:
        deployment_region = os.environ.get('AWS_REGION')
