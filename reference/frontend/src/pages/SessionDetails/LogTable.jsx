...
export function LogTable({sessionId, notifyFn}) {
  ...
  useEffect(() => {   
    setLoading(true);
    axiosInstance.get("/sessions/" + sessionId + "/logs")
      .then(response => {
      ...
      .catch(error => {
        console.log(error);
        notifyFn('error', `Unable to get the ${sessionId} session logs.`);
      });
  }, [refreshKey]);
  ...
  return (
    ...
    <Table
      ...
      loading={loading}
      loadingText="Loading session logs..."
      items={logs}
